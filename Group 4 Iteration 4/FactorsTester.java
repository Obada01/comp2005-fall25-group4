import static org.junit.jupiter.api.Assertions.*;

import java.util.ArrayList;
import java.util.Arrays;

import org.junit.jupiter.api.Test;

class FactorsTester {

    @Test
    void testPerfect1()
    {    
        // TEST 1: should throw the exception because the parameter value is less than 1
        assertThrows(IllegalArgumentException.class, () -> FactorsUtility.perfect(0));
    }
    
    @Test
    void testPerfect2()
    {    
        // TEST 2: should succeed because 1 is a valid parameter value, but is not a perfect number
        assertFalse(FactorsUtility.perfect(1));
    }
    
    @Test
    void testPerfect3()
    {    
        // TEST 3: should succeed because 6 is a valid parameter value, and is a perfect number
        assertTrue(FactorsUtility.perfect(6));
    }
    
    @Test
    void testPerfect4()
    {    
        // TEST 4: should succeed because 7 is a valid parameter value, but is not a perfect number
        // I've coded this using assertEquals to show that there's often more than one way to write a test 
        boolean expected = false;
        assertEquals(expected, FactorsUtility.perfect(7));
    }

    @Test
    void testFactor1()
    {
        //Should throw cause a < 0
        assertThrows(IllegalArgumentException.class, () -> FactorsUtility.factor(-1,2));
    }
    
    @Test
    void testFactor2()
    {
        //Should throw cause b < 1
        assertThrows(IllegalArgumentException.class, () -> FactorsUtility.factor(1,0));
    }
    
    @Test
    void testFactor3()
    {
        //Should assert true as 1 is a factor of any real number a 
        assertTrue(FactorsUtility.factor(2,1));
    }
    
    @Test
    void testFactor4()
    {
        //Should assert true as any real number b is a factor of 0
        assertTrue(FactorsUtility.factor(0,2));
    }
    
    @Test
    public void testFactor5()
    {
        //Should assert true as 2 is a factor of 2. 
        assertEquals(true, FactorsUtility.factor(2, 2));
    }
    
    @Test
    void testGetFactor1()
    {
        //Should throw as a < 0
        assertThrows(IllegalArgumentException.class, () -> FactorsUtility.getFactors(-1));
    }
    
    @Test
    void testGetFactor2()
    {
        //Should assert an empty list as a = 0
        assertEquals(Arrays.asList(), FactorsUtility.getFactors(0));
    }
    
    @Test
    void testGetFactor3()
    {
        //Should assert an empty list as a = 1
        assertEquals(Arrays.asList(), FactorsUtility.getFactors(1));
    }
    
    @Test
    void testGetFactor4()
    {
        //Should assert an int list containing only [1] in it as 2 has only 1 as its factor (excluding itself)
        assertEquals(Arrays.asList(1), FactorsUtility.getFactors(2));
    }
    
    @Test
    void testGetFactor5()
    {
        //Should assert an int list containing [1,2,3] as those are all of the factors of 6 excluding itself
        assertEquals(Arrays.asList(1,2,3), FactorsUtility.getFactors(6));
    }
}



